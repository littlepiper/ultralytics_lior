...
class YOLOWeightedDataset(YOLODataset):
    ...
    def get_virtual_samples(self):
        """
        Generate equivalent distribution data through virtual resampling (simplified sequential version)
        Args:
            num_epoch_samples (int): Theoretical total number of samples per epoch
        Returns:
            (np.ndarray, np.ndarray): Equivalent boxes and cls arrays
        """
        # Generate deterministic random sampling indices
        np_random = np.random.RandomState(seed=42)  # Fixed random seed for reproducibility
        sampled_indices = np_random.choice(
            len(self.labels),
        ...
        # Sequentially process each sampled index
        cls_list, boxes_list = [], []
        for idx in sampled_indices:
            label = self.labels[idx]
            cls_list.append(label['cls'].reshape(-1).astype(int))
            boxes_list.append(label['bboxes'])

        # Combine results while preserving original format
        virtual_cls = np.concatenate(cls_list)
        virtual_boxes = np.concatenate(boxes_list)
        return virtual_boxes, virtual_cls
